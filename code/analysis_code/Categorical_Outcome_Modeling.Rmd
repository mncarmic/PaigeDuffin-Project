...
# Bioregion
```{r}
bioregion_histogram <- clean_data %>% ggplot() + geom_bar(aes(bioregion),fill=bioreg_colours)
bioregion_histogram 
...
```{r}
clean_data$season_sequence <- as.factor(clean_data$season_sequence)
...
Single_Predictor_Accuracy <- Singlepred_df_ordered %>% ggplot(aes(x = Variable, y=Accuracy, label =Accuracy)) + geom_hline(yintercept=null_accuracy_treefit,color='red', size=2,linetype="dashed") +   geom_label(stat='identity',color="black", size=4,fill="lightgrey") + theme(title = element_text(size=12), axis.title.y = element_text(size=14),axis.title.x = element_blank(),axis.text.y = element_text(size=12),axis.text.x = element_text(size=8,angle=40,hjust=1)) + ggtitle("Single Predictor Models (pre-subsetting)") + geom_vline(xintercept=11.5,color="deepskyblue",size=1.5,linetype="dashed")
...
measureACC(train_data_results$Predicted, train_data_results$Actual)
...
measureACC(test_data_results$Predicted, test_data_results$Actual)
#use predicted and actual outcomes, make a table and compute accuracy.
```
