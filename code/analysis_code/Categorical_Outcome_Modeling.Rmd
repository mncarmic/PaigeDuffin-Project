...
Single_Predictor_Accuracy <- Singlepred_df_ordered %>% 
  ggplot(aes(x = Variable, y=Accuracy, label =Accuracy)) + 
  geom_hline(yintercept=null_accuracy_treefit,color='red', size=2,linetype="dashed") +
  geom_vline(xintercept=11.5,color="deepskyblue",size=1.5,linetype="dashed") +
  geom_label(stat='identity',color="black", size=4,fill="lightgrey") + 
  theme(title = element_text(size=12), axis.title.y = element_text(size=14),axis.title.x = element_blank(),axis.text.y = element_text(size=12),axis.text.x = element_text(size=8,angle=40,hjust=1))
